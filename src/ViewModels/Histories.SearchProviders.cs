    ...
    partial class Histories
    {
        ...
        public void AddFilter(string token)
        {
            ...
            try
            {
                ...
                var current = SearchTokens.FirstOrDefault(x => x.Raw.Equals(token, StringComparison.OrdinalIgnoreCase));
                if (current != null)
                    return;

                var opposite = isNegated ? checkStr : $"-{checkStr}";
                var old = SearchTokens.FirstOrDefault(x => x.Raw.Equals(opposite, StringComparison.OrdinalIgnoreCase));
                if (old != null)
                {
                    SearchTokens.Remove(old);
                }
        ...
        private void SetupSearchProviders()
        {
            ...
            // b:（本地分支）建议器 — 持久过滤器 | Suggester: Local Branches
            // 数据来源: _repo.Branches（Repository 中的本地分支列表）
            // 与旧实现不同：不再从 _commits.Decorators 中提取（旧方案只显示已加载提交的分支），
            // 而是直接取仓库中所有 IsLocal==true 的分支。
            // 因为 b: 是 priority:0 的持久过滤器，缓存 = _rawCommits，
            // 且不参与 EvaluateToBitmap（在 skip-list 中跳过），
            // 所以不需要 _suggestionCache，直接读 _repo 即可覆盖全量分支。
            ...
            SearchProviders.Add(
                ...
                icon: App.GetIcon("Icons.Branch"), priority: 0, isPersistent: true));
            ...
            SearchProviders.Add(new StaticTokenSuggestionProvider(
                ...
                priority: 31));
            SearchProviders.Add(new StaticTokenSuggestionProvider(
                "e:", "邮箱", groupAdvanced, alias: new[] { "email:" },
        ...
        private void OnSearchTokensChanged(object sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
        {
            ...
            foreach (var token in SearchTokens)
            {
                         ...
                         check.StartsWith("remote:", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        var type =
                            val.Contains('/') ? Models.FilterType.RemoteBranch : Models.FilterType.RemoteBranchFolder;
        ...
        private BitArray EvaluateToBitmap(Controls.GroupSpec group)
        {
            ...
            if (group.ProviderPrefix == "solo:")
            {
                ...
                foreach (var target in soloTargets)
                {
                    ...
                    {
                        var head = _rawCommits.Find(x => x.IsCurrentHead);
                        if (head != null)
                            sha = head.SHA;
                    }

                    if (_commitMap.TryGetValue(sha, out var commit))
                    {
                        var lineage = Models.CommitGraph.GetCommitLineageFast(_rawCommits, _commitMap, commit,
