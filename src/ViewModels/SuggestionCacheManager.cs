...
namespace SourceGit.ViewModels
{
    internal class SuggestionCacheManager
    {
        ...
        private readonly Func<string, Models.Commit, Controls.ExprNode, bool> _evalTerm;

        // Atomic bitmap cache: (Prefix, Value) -> BitArray
        private readonly Dictionary<(string, string), BitArray> _atomCache = new();

        public SuggestionCacheManager(IEnumerable<Controls.ITokenSuggestionProvider> providers,
                                      Func<string, Models.Commit, Controls.ExprNode, bool> evalTerm)
        {
        ...
        private BitArray GetAtomBitmap(string prefix, Controls.ExprNode node, List<Models.Commit> rawCommits)
        {
            var key = (prefix, node.Value);
            ...
            {
                bits[i] = _evalTerm(prefix, rawCommits[i], node);
            }
        ...
        private BitArray EvaluateExpr(Controls.ExprNode node, string prefix, List<Models.Commit> rawCommits)
        {
            ...
            switch (node.Op)
            {
                case Controls.ExprOp.Term:
                    return GetAtomBitmap(prefix, node, rawCommits);
        ...
        public Dictionary<string, List<Models.Commit>> Compute(List<Models.Commit> rawCommits, List<Models.Commit> commits,
                                                               Controls.QuerySpec spec)
        {
            ...
            var persistentPrefixes = new[] { "b:", "t:", "r:", "gitlog:", "solo:" };
